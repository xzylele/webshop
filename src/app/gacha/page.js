    ...
    const loop = (timestamp) => {
      ...
      if (progress < 1) {
        ...
        if (result.type !== 'empty') {
          playWinSound();
          setTimeout(() => confetti.launch(), 200);
        }
    ...
    <div className="relative min-h-screen flex flex-col bg-[#02060d]">
        ...
        <div className={`lg:col-span-2 flex flex-col items-center justify-center space-y-6 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div
            ...
            style={{
              ...
              boxShadow: spinning
                ? `0 0 ${40 + glowIntensity * 60}px rgba(56,189,248,${glowIntensity}), 0 0 ${80 + glowIntensity * 100}px rgba(56,189,248,${glowIntensity * 0.3}), inset 0 0 30px rgba(56,189,248,${glowIntensity * 0.1})`
                : '0 0 30px rgba(56,189,248,0.05), inset 0 0 20px rgba(56,189,248,0.02)',
            }}
          >
            ...
            <div
              ...
              style={{
                background: `conic-gradient(from 0deg, transparent, rgba(56,189,248,${spinning ? 0.1 : 0.03}), transparent, rgba(167,139,250,${spinning ? 0.08 : 0.02}), transparent)`,
            ...
            <div className={`absolute -top-1.5 left-1/2 -translate-x-1/2 z-20 transition-transform ${spinning ? 'animate-[indicatorBounce_0.3s_ease-in-out_infinite]' : ''}`}>
              <div className="w-0 h-0 border-l-[14px] border-l-transparent border-r-[14px] border-r-transparent border-t-[22px] border-t-sky-400 filter drop-shadow-[0_2px_8px_rgba(56,189,248,0.5)]" />
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full justify-center">
            ...
            <button
              ...
              className={`w-full sm:w-[220px] flex items-center justify-center gap-1.5 font-black py-3.5 px-6 rounded-xl disabled:opacity-50 disabled:pointer-events-none transition-all text-xs tracking-wider cursor-pointer ${
                spinning
                  ? 'bg-sky-600 text-sky-100 animate-pulse'
                  : 'bg-sky-500 text-sky-950 hover:bg-sky-400 hover:scale-[1.03] active:scale-[0.97] glow-btn hover:shadow-[0_0_40px_rgba(56,189,248,0.4)]'
              }`}
            >
              {spinning ? (
                <>
      ...
      {spinResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-[fadeIn_0.3s_ease-out]">
          ...
          <div className="relative w-full max-w-md bg-[#060c13] border border-sky-500/20 p-6 rounded-2xl shadow-2xl glass z-10 text-center overflow-hidden animate-[popIn_0.4s_cubic-bezier(0.175,0.885,0.32,1.275)]">
            
            {/* Glow behind popup */}
            <div className={`absolute -top-12 -right-12 w-40 h-40 rounded-full blur-3xl ${spinResult.type === 'empty' ? 'bg-zinc-500/5' : 'bg-sky-500/10 animate-pulse'}`} />
            <div className={`absolute -bottom-12 -left-12 w-40 h-40 rounded-full blur-3xl ${spinResult.type === 'empty' ? 'bg-zinc-500/5' : 'bg-purple-500/10 animate-pulse'}`} />

            {spinResult.type === 'empty' ? (
              /* EMPTY RESULT */
              <div className="space-y-5 py-4">
                <div className="inline-flex bg-zinc-800 border border-zinc-700 p-4 rounded-full text-zinc-400 mx-auto animate-[wobble_0.8s_ease-in-out]">
                ...
                <button
                  ...
                  className="w-full bg-zinc-800 text-zinc-300 hover:text-white py-3 rounded-xl font-bold text-xs cursor-pointer transition-colors hover:bg-zinc-700"
                >
                  ปิดหน้าต่าง
                </button>
              </div>
              ...
              <div className="space-y-5 py-4">
                <div className="inline-flex bg-sky-500/10 border border-sky-500/20 p-4 rounded-full text-sky-400 mx-auto animate-[bounceIn_0.6s_cubic-bezier(0.175,0.885,0.32,1.275)]">
                  <CheckCircle className="w-12 h-12" />
                </div>
                
                <div className="space-y-1.5 animate-[fadeSlideUp_0.5s_ease-out_0.2s_both]">
                ...
                {/* Received code box */}
                <div className="bg-zinc-950/60 border border-white/5 p-4 rounded-xl text-left space-y-2 animate-[fadeSlideUp_0.5s_ease-out_0.35s_both]">
                  ...
                  <div className="flex items-center justify-between gap-3 bg-white/5 border border-white/5 px-3.5 py-2.5 rounded-lg text-xs font-mono text-zinc-100 break-all select-all hover:border-sky-500/20 transition-colors">
                    <span>{spinResult.wonValue}</span>
                    <button
                      onClick={() => handleCopy(spinResult.wonValue)}
                      className="shrink-0 text-sky-400 hover:text-sky-300 p-1.5 hover:bg-white/5 rounded transition-colors cursor-pointer active:scale-90"
                    >
                      {copied ? (
                ...
                <p className="text-[10px] text-zinc-500 animate-[fadeSlideUp_0.5s_ease-out_0.5s_both]">
                  * หากเป็นคูปองสามารถก๊อปปี้ไปป้อนใส่ช่องลดราคาตอนซื้อสินค้าได้ทันที ประวัติบันทึกลงในบิลทรานแซกชั่นแล้ว
                </p>

                <button
                  onClick={() => setSpinResult(null)}
                  className="w-full bg-sky-500 text-sky-950 hover:bg-sky-400 py-3 rounded-xl font-bold text-xs cursor-pointer transition-all glow-btn hover:shadow-[0_0_30px_rgba(56,189,248,0.4)] active:scale-[0.97] animate-[fadeSlideUp_0.5s_ease-out_0.6s_both]"
                >
                  รับรางวัล
                </button>
              </div>
